 ...
 */
static uint8_t TxDutyCycleTimeQuery = 0;
...
static void PrepareTxFrame( uint8_t port )
{
	...
	if(TxDutyCycleTimeQuery) 
	{
		TxDutyCycleTimeQuery = 0;
	}
	else
	{
		relay_read_pins_status(GPIOA, AppData, &AppDataSize);
	}
//	printf("%x %x %x\r\n",AppData[0], AppData[1], AppData[2]); // Debug Code
}
...
//			printf("DownLinkCounter:%d\r\n",mcpsIndication->DownLinkCounter); // Debug Code
			...
			if(ActionCmdCounter < mcpsIndication->DownLinkCounter)
			{
				...
				// Action
				relay_instruction_analysis(mcpsIndication->Buffer[0], mcpsIndication->Buffer[1]);
				TxDutyCycleTimeChange = TxDutyCycleTime_instruction_analysis(&TxDutyCycleTime, mcpsIndication->Buffer[0],\
																				...
																				mcpsIndication->Buffer[3], AppData, &AppDataSize,&TxDutyCycleTimeQuery);
